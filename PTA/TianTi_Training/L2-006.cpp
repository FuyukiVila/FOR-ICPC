...
void find(const vector<int> &h, const vector<int> &z, int deep) {
    if (h.empty() || z.empty())return;
    vector<int> lh, rh, lz, rz;
    int root = h.back();
    ans[deep].push_back(root);
    bool flag = false;
    for (int i = 0, j = 0; i < z.size(); i++,j++) {
        if(root == z[i]){
            flag = true;
            j--;
            continue;
