...
void idol_produce(int testCase) {
    ...
    int l = 0, r = *max_element(a.begin() + 1, a.end()) - *min_element(a.begin() + 1, a.end());
    auto get_min = [&](int x, multiset<int> &s) {
        int res = abs(*s.begin() - x);
        auto p = s.lower_bound(x); // >= x
        auto q = s.upper_bound(x); // < x
        if (p != s.end()) {
            res = min(res, abs(x - *p));
        }
        if (q != s.begin()) {
    ...
    auto check = [&](int maxn) -> pair<int, multiset<int>> {
        if (abs(a[1] - a[2]) > maxn) {
            return {0, {}};
        }
        multiset<int> s{a[2]};
        int tmp = a[1];
        for (int i = 3; i <= n; i++) {
            if (get_min(a[i], s) > maxn && abs(tmp - a[i]) > maxn) {
                return {0, {}};
            } else if (get_min(a[i], s) <= maxn && abs(tmp - a[i]) <= maxn) {
                s.insert(tmp);
                tmp = a[i];
            } else if (abs(tmp - a[i]) <= maxn) {
                tmp = a[i];
            } else {
                s.clear();
                s.insert(a[i]);
