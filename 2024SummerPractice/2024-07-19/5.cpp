...
void idol_produce(int testCase) {
    ...
    int n, m;
    cin >> n >> m;
    vector<vector<pair<int, int >>> tr(n * 2 + 5);
    auto update = [&](int a, int b, int l, int r) {
        for (l += n, r += n; l < r; l >>= 1, r >>= 1) {
            if (l & 1) {
                tr[l].emplace_back(a, b);
                l++;
            }
            if (r & 1) {
                r--;
                tr[r].emplace_back(a, b);
            }
        }
    };
    ...
    auto dfs = [&](auto &dfs, int u) -> void {
        ...
        for (auto const &[a, b]: tr[u]) {
            dsu.unite(a, b);
        }
        if (n <= u && u < n * 2) {
            dsu.push_tag(dsu.find(0), u - n + 1);
        }
        dfs(dfs, u * 2);
        dfs(dfs, u * 2 + 1);
