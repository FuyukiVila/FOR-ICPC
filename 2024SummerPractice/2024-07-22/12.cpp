...
struct DSU {
    ...
    explicit DSU(int n) : f(n + 1), siz(n + 1, 1), h(n + 1), son(n + 1) {
...
void idol_produce(int testCase) {
    ...
    int n, m, d, k;
    cin >> n >> m >> d >> k;
    vector<DSU> dsu(1, DSU(n));
    int ans = 0;
    for (int i = 1; i <= m; i++) {
        int x, y;
        cin >> x >> y;
        int fx = dsu.back().find(x), fy = dsu.back().find(y);
        if (fx == fy)continue;
        if (dsu.back().size(fx) < dsu.back().size(fy)) {
            swap(fx, fy);
            swap(x, y);
        }
        ans += dsu.back().size(fy) * dsu.back().size(fx);
        dsu.back().siz[fx] += dsu.back().siz[fy];
        dsu.back().f[fy] = fx;
        for_each(dsu.back().son[fy].begin(), dsu.back().son[fy].end(),
                 [&](int x) { dsu.back().son[fx].emplace_back(x); });
        dsu.back().son[fy].clear();
    }

    for (int i = 1; i <= d + 1; i++) {
        dsu.emplace_back(dsu.back());
    }
    ...
    for (int i = 1; i <= d + 1; i++) {
        for (int j = 1; j <= n; j++) {
            int p = dsu[0].find(j);
            dsu[i].f[j] = p;
            dsu[i].siz[j] = dsu[0].siz[p];
            h[j] += dsu[i].h[p];
            if (j == p) {
                dsu[i].son[j] = dsu[0].son[j];
            } else {
                dsu[i].son[j].clear();
            }
        }
    ...
    while (k--) {
        int x, y, w;
        cin >> x >> y >> w;
        int fx = dsu[w].find(x), fy = dsu[w].find(y);
        if (fx == fy) {
            cout << ans << '\n';
            continue;
        }
        if (dsu[w].size(fx) < dsu[w].size(fy)) {
            swap(fx, fy);
            swap(x, y);
        }
        unordered_map<size_t, int> nmap;
        for (auto const &i: dsu[w].son[fy]) {
            hmap[h[i]]--;
            if (hmap[h[i]] == 0) {
                hmap.erase(h[i]);
            } else {
            ...
            h[i] += dsu[w].h[fx] - dsu[w].h[fy];
            nmap[h[i]]++;
            dsu[w].son[fx].emplace_back(i);
        }
        dsu[w].siz[fx] += dsu[w].siz[fy];
        dsu[w].f[fy] = fx;
        dsu[w].son[fy].clear();
        for (auto const &[key, value]: nmap) {
            if (hmap.count(key)) {
                ans += hmap[key] * value;
            }
            ans += value * (value - 1) / 2;
