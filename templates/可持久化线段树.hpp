...
namespace pst {
    ...
    protected:
        ...
        void push_up(int node) {
            tree[node].value = merge_value(tree[tree[node].ls].value, tree[tree[node].rs].value);
        }

        void build(const std::vector<element_type> &arr, size_t &node, size_t l, size_t r) {
            node = ++total;
            if (l == r) {
        ...
        size_t _update(size_t &node, size_t last, size_t pos, element_type val, size_t l, size_t r) {
            node = ++total;
            tree[node] = tree[last];
            if (l == r) {
                tree[node].value = val;
            ...
            int mid = (l + r) >> 1;
            if (pos <= mid) _update(tree[node].ls, tree[last].ls, pos, val, l, mid);
            else _update(tree[node].rs, tree[last].rs, pos, val, mid + 1, r);
            push_up(node);
            return node;
        }
        ...
        value_type _get(size_t node, size_t pos, size_t l, size_t r) {
            ...
            int mid = (l + r) >> 1;
            if (pos <= mid) return _get(tree[node].ls, pos, l, mid);
            else return _get(tree[node].rs, pos, mid + 1, r);
        }
    ...
    class PersistentSegmentTree : public IPersistentSegmentTree<long long, long long> {
    private:
        value_type assign_value(const std::vector<element_type> &arr, size_t x) {
            return arr[x];
        }

        value_type merge_value(value_type x, value_type y) {
            return x + y;
        }
    ...
    private:
        ...
        value_type _kthMin(size_t prev, size_t now, size_t l, size_t r, size_t k) {
            ...
            if (k <= x) return _kthMin(tree[prev].ls, tree[now].ls, l, mid, k);
            else return _kthMin(tree[prev].rs, tree[now].rs, mid + 1, r, k - x);
        }
