...
namespace Dimension2 {
    ...
    public:
        ...
        // 数乘
        constexpr Vector operator*(double b) const { return {x * b, y * b}; }
        ...
        // 模
        constexpr double norm() const { return sqrt(*this * *this); }

        // 点距
        constexpr double distance(Point p) const { return (*this - p).norm(); }
    };
    ...
    public:
        ...
        constexpr bool seg_inter_seg(const Seg &l) const {
            ...
            return res == 2 && pointOnSeg(intersectionPoint) &&
                   l.pointOnSeg(intersectionPoint);
        }
...
// 计算多边形面积
    double calcArea(const vector<Point> &p) {
...
// 对点p极角排序，按逆时针排序
    ...
    public:
        ...
        constexpr static int quadrant(const Point &a) {
        ...
        bool operator()(const Point &a, const Point &b) const {
            if (quadrant(a - p) != quadrant(b - p)) {
                return quadrant(a - p) < quadrant(b - p);
            }
            if (sgn((a - p) ^ (b - p)) == 0) {
...
// Andrew求凸包
// 返回逆时针排序的点, poly[n] = poly[0]
    vector<Point> andrew(vector<Point> p) {
        sort(p.begin(), p.end(), [](const Point &a, const Point &b) {
            return a.x < b.x || (a.x == b.x && a.y < b.y);
        });
        int tp = 0;
        vector<int> stk(p.size() + 5);
        vector<int> used(p.size(), false);
        stk[++tp] = 0;
        for (int i = 1; i < p.size(); i++) {
            while (tp >= 2 &&
                   sgn((p[stk[tp]] - p[stk[tp - 1]]) ^ (p[i] - p[stk[tp]])) <= 0) {
                used[stk[tp--]] = 0;
            }
            used[i] = 1;
            stk[++tp] = i;
        ...
        for (int i = p.size() - 2; i >= 0; i--) {
            if (!used[i]) {
                while (tp > tmp && sgn((p[stk[tp]] - p[stk[tp - 1]]) ^
                                       (p[i] - p[stk[tp]])) <= 0) {
                    used[stk[tp--]] = 0;
                }
                used[i] = 1;
...
// 点可以是顺时针给出也可以是逆时针给出
    bool isConvex(const vector<Point> &poly) {
        ...
        bitset<3> s;
        for (int i = 0; i < n; i++) {
            s[sgn((poly[(i + 1) % n] - poly[i]) ^ (poly[(i + 2) % n] - poly[i])) +
              1] = true;
...
//  凸包上的线段两两不能相交
    bool isConvexHullSeparate(const vector<Point> &a, const vector<Point> &b) {
        ...
        for (int i = 0; i < n; i++)
            if (PointInPoly(a[i], b) != -1) return false;

        for (int i = 0; i < m; i++)
            if (PointInPoly(b[i], a) != -1) return false;

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                Line l1 = Line(a[i], a[(i + 1) % n]);
                Line l2 = Line(b[j], b[(j + 1) % m]);
                if (l1.seg_inter_seg(l2)) return false;
            }
        }
        return true;
    }
...
// 旋转卡壳求凸包直径
// 逆时针存点, poly[n] = poly[0]
    double rotatingCalipers(const vector<Point> &poly) {
        if (poly.size() <= 2) {
            return 0;
        }
        if (poly.size() == 3) {
            return poly[0].distance(poly[1]);
        }
        double ans = 0;
        int cur = 0;
        for (int i = 0; i < poly.size() - 1; i++) {
            Line line(poly[i], poly[i + 1]);
            while (line.distance(poly[cur]) <=
                   line.distance(poly[(cur + 1) % poly.size()])) {
                cur = (cur + 1) % poly.size();
            }
            ans = max(ans, max(poly[i].distance(poly[cur]),
                               poly[i + 1].distance(poly[cur])));
        }
        return ans;
    }
}  // namespace Dimension2
...
namespace Dimension3 {
    ...
    public:
        ...
        // 数乘
        constexpr Vector operator*(double b) const { return {x * b, y * b, z * b}; }
    ...
    public:
        ...
        // 线段长度
        constexpr double length() const { return (b - a).norm(); }
        ...
        constexpr std::pair<int, Point> operator&(const Line &l) const {
            ...
            if (!pointOnLine(intersectionPoint) ||
                !l.pointOnLine(intersectionPoint)) {
                return {3, Point()};
            }
        ...
        constexpr bool pointOnSeg(const Point &p) const {
            ...
            return pointOnLine(p) && p.x >= min(a.x, b.x) && p.x <= max(a.x, b.x) &&
    ...
    public:
        ...
        // 有参构造函数
        constexpr Plane(const Point &a, const Point &b, const Point &c)
                : a(a), b(b), c(c) {}
        ...
        constexpr double distance(const Point &p) const {
            ...
            return abs(normal.x * p.x + normal.y * p.y + normal.z * p.z + d) /
                   normal.norm();
        }
        ...
        constexpr pair<int, Point> intersectWithLine(const Line &line) const {
            ...
            // 计算交点
            double t =
                    -((normal.x * (line.a.x - a.x) + normal.y * (line.a.y - a.y) +
                       normal.z * (line.a.z - a.z)) /
                      (normal * (line.b - line.a)));
            return {1, line.a + (line.b - line.a) * t};
        }

        // 面和面的交线，0为不存在，1为存在
        constexpr std::pair<int, Line> intersectWithPlane(
